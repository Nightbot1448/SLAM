...
int main(int  argc, char **argv) {
    ...
    std::ifstream in1("/home/dmo/Documents/dimplom/dumps/compressed_dump_0.txt");
    ...
    // DiscretePoint2D end_of_map = DiscretePoint2D(map_0.width, map_0.height) - origin;
    ...
    for (pnt.y = -origin.y; pnt.y < end_of_map.y; ++pnt.y) {
        for (pnt.x = -origin.x; pnt.x < end_of_map.x; ++pnt.x) {
            ...
            VinyDSCell cell = VinyDSCell(static_cast<double>(TBM_to_O(res)), res);
            
            map.setCell(pnt, cell);

            value = static_cast<double>(map[pnt]);
